...
import { readdirSync, rmSync, mkdirSync, readFileSync, writeFileSync, existsSync } from "fs";
...
function wrapWithBYOK(content: string, metadata: ExampleMetadata): string {
  ...
  for (const line of lines) {
    ...
    if (!pastImports && line.trim().startsWith('import ')) {
      inImport = true;
      const updatedLine = line.replace(/from ['"]\.\.\//g, 'from "@qforge/torque');
...
`;
  ...
  const updatedCode = codeLines.join('\n')
    ...
    .replace(/model: openai\("gpt-5-mini"/g, 'model: openai("gpt-4o-mini"'); // Fix typo if exists
 ...
 */
function processTemplate(templateContent: string, metadata: ExampleMetadata): string {
...
function generateTemplate(exampleFile: string) {
  ...
  const templateFiles = ["package.json", "tsconfig.json", ".stackblitzrc", "README.md"];
